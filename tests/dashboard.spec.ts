...
test.describe("Pozo pages", () => {
  test("nuevo pozo page renders", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Nuevo Pozo" }),
    ).toBeVisible();
...
test.describe("Jugadores page", () => {
  test("renders player form", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Añadir jugador" }),
    ).toBeVisible();
  });
...
test.describe("Sorteo page", () => {
  test("renders sorteo page with empty state and draw buttons", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Necesitas al menos 4 jugadores/),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Aleatorio", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Aleatorio Mixto", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Por Nivel", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Por Nivel Mixto", exact: true }),
    ).toBeVisible();
  });
});
