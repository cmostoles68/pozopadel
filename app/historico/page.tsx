...
interface Match {
  ...
  winner: [PlayerSnap, PlayerSnap];
  loser: [PlayerSnap, PlayerSnap];
}

export default async function HistoricoPage() {
  const supabase = await createClient();

  const [{ data: history }, { data: tournaments }, { data: profiles }] =
    await Promise.all([
      supabase.from("pozo_match_history").select("*").order("created_at", { ascending: false }),
      supabase
        .from("tournaments")
        .select("id, title, status, champion_drawn_pair_id")
  ...
  // Champion players per tournament: resolved from the history row whose
  // winner_drawn_pair_id matches the tournament's champion_drawn_pair_id.
  ...
  const matches: Match[] = (history ?? []).map((h) => ({
    id: h.id,
  ...
  // Aggregate unique players that have ever played (from history), keeping the
  // most complete data snapshot.
