  ...
  return (
    <div className="min-h-screen bg-background text-on-surface font-sans">
      ...
      <nav className="md:hidden fixed top-0 inset-x-0 z-50 flex items-center justify-between px-4 py-3 bg-surface/60 backdrop-blur-xl border-b border-outline-variant/20">
        <Link href="/dashboard" className="font-display text-lg font-bold tracking-tight text-secondary-fixed-dim">
          PadelElite
        </Link>
      ...
      <aside className="hidden md:flex flex-col bg-surface-lowest h-screen w-64 fixed left-0 top-0 z-40 border-r border-outline-variant/10">
        ...
        <div className="p-4 border-t border-outline-variant/10">
          <Link
            href="/auth/login"
            className="mx-2 my-1 flex items-center px-4 py-2 rounded-full text-on-surface-variant hover:text-on-surface hover:bg-surface-high/40 transition-all"
          >
