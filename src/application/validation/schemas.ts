...
export const createPlayerSchema = z.object({
  ...
  dominant_hand: z.enum(["RIGHT", "LEFT"], {
    message: "Mano dominante no válida",
  }),
  level: z.coerce
    .number()
...
export const createTournamentSchema = z.object({
  ...
  minutesPerRound: z.coerce
    .number()
    .int()
    .min(1, "Los minutos por ronda deben ser entre 1 y 90")
    .max(90, "Los minutos por ronda deben ser entre 1 y 90"),
});

export const drawMethodSchema = z.enum([
  "random",
  "random_mix",
  "level",
  "level_mix",
...
export const courtResultSchema = z
  .array(
    z.object({
...
export const adminLoginSchema = z.object({
  password: z.string().trim().min(1, "La contraseña es obligatoria").max(200),
});
